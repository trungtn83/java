...
public class QuestTower2 extends Quest {
	...
	public void execute() throws Exception {
		while (true) {
			...
			println("Hien tai dang co " + potentialPoint + "diem tiem nang.");
			...
			if (StringUtils.contains(towerShowInfoLastURI, "drop")) {
				...
				println("VUA NHAN DUOC THE ROI RA !!!");
				continue;
			}
...
//					Utils.println(bossDocument);
					...
					println("BAN VUA DANH BOSS XONG");
...
//					}
				...
				if (towerQuest.getManaHad() + potentialPoint >= towerQuest.getManaCost()) {
					...
					if (!isUsedMana) {
						...
						println("HET NGOC : DOI " + waitTime + " PHUT DE HOI PHUC MANA !!!");
...
//							Utils.println("NEU AN TINH LUC SE THUA DIEM DE LEN LEVEL");
							Long waitTime = (towerQuest.getManaCost() - towerQuest.getManaHad())*2;
							println("AN TINH LUC THI THUA EXP : DOI " + waitTime + " PHUT DE HOI PHUC MANA !!!");
//							Thread.sleep(waitTime*60*1000);
						}
					}
